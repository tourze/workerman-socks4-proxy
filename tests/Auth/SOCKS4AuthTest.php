...
class SOCKS4AuthTest extends TestCase
{
    private ?SOCKS4Auth $auth = null;
    ...
    public function testGetInstance_ReturnsSameInstance()
    {
    ...
    public function testSetEnableAuthentication_ChangesAuthenticationState()
    {
        // 测试启用/禁用验证状态更改
        $this->auth->setEnableAuthentication(true);
    ...
    public function testAddValidUser_AutoEnablesAuthentication()
    {
        // 添加有效用户时应自动启用验证
        $this->auth->setEnableAuthentication(false);
        $this->assertFalse($this->auth->isAuthenticationEnabled());
    ...
    public function testSetValidUsers_ReplacesAllUsers()
    {
        // 先添加一个用户
    ...
    public function testGetValidUsers_ReturnsUsersList()
    {
        // 添加几个有效用户
        $this->auth->addValidUser('user1');
